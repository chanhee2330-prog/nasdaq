...
if mode == "📊 단일 전략 백테스트":
    ...
    if single_strategy == "추세추종 (MA) ⭐추천":
        sp["window"] = st.sidebar.slider("이동평균 기간 (일)", 3, 300, 250,
                                         help="종가가 이 기간의 이동평균선 '위'면 보유, '아래'면 현금. "
                                              "전체기간(1971~) 분석에서 250이 가장 우수했습니다.")
...
| 전략 | **추세추종(MA) 250 + 버퍼 5%** ⭐ | 아래 설명 참고 |

### ⭐ 추천 전략: 추세추종 (250일 이동평균 + 버퍼 밴드)
> **규칙**: 신호 종목의 종가가 **250일 이동평균선을 5% 이상 확실히 넘으면 SOXL 보유(매수), 5% 아래로 내려가면 전량 매도(현금)**.
>
...
with st.expander("📚 용어 사전 — 어려운 단어/영어 용어 설명 (클릭)"):
    st.markdown(
        """
#### 📊 결과 지표 (숫자 읽는 법)
- **총수익률**: 기간 전체 누적 손익. +500% = 원금의 6배가 됨.
- **CAGR** (씨에이지알, Compound Annual Growth Rate, 연평균 복리 수익률): **"매년 평균 몇 %씩 불었나"**.
  &nbsp;👉 예: 10년 동안 원금이 2배(+100%)가 됐다면 CAGR은 약 **7%** (매년 7%씩 복리로 쌓인 셈). 총수익률은 기간이 길수록 무조건 커지므로, 기간이 다른 전략을 공정하게 비교할 땐 CAGR을 봅니다.
- **변동성** (Volatility): 수익이 출렁이는 정도. 클수록 등락이 심함.
- **샤프 지수** (Sharpe Ratio): **위험(출렁임) 대비 수익 효율**. "출렁임을 얼마나 감수하고 그만큼 벌었나".
  &nbsp;👉 예: 샤프 **1.0=양호, 2.0=훌륭, 0.5 이하=효율 낮음**. 수익이 같아도 **덜 출렁이며 번 쪽**의 샤프가 높습니다. (운에 덜 기댄, 더 '편한' 수익)
- **MDD** (Maximum Drawdown, 최대 낙폭): 고점 대비 **최대 하락폭**. -50%면 반토막까지 갔다는 뜻. **0에 가까울수록 안전**.
- **bp** (basis point, 베이시스 포인트): 0.01%. 매매 수수료 단위. 5bp = 0.05%.

#### 📈 전략·매매 용어
- **백테스트** (Backtest): 과거 데이터에 매매 규칙을 적용해 "그때 이렇게 했으면 어땠을까"를 계산.
- **매수 후 보유** (Buy & Hold): 그냥 사서 끝까지 들고 있는 것. 모든 전략의 비교 기준.
- **추세추종** (Trend Following): 오를 때 따라 사고, 꺾이면 파는 방식.
- **이동평균선 / MA** (Moving Average): 최근 N일 평균 가격을 이은 선. 추세 판단의 기본 도구.
- **버퍼 밴드** (Buffer Band): 이동평균선을 살짝(예: 5%) 확실히 넘을 때만 매매. 선 근처에서 사고팔길 반복하는 **휩쏘(whipsaw, 잦은 헛매매)**를 줄임.
- **돌파** (Breakout) / **돈치안 채널** (Donchian Channel): 최근 N일 최고가를 뚫고 오르면 매수하는 방식.
- **트레일링 스톱** (Trailing Stop): 가격이 오르면 손절선도 같이 따라 올려, 수익은 길게 두고 꺾일 때만 파는 장치.
- **RSI** (Relative Strength Index, 상대강도지수): 0~100. 너무 오르면(>70) 과매수, 너무 내리면(<30) 과매도로 봄.
- **ATR** (Average True Range): 하루 평균 변동폭. 손절 간격을 변동성에 맞출 때 씀.
- **ADX**: 추세가 얼마나 강한지(방향 무관) 나타내는 값. 높을수록 추세가 뚜렷.
- **이격도** (Disparity): 현재가가 이동평균선에서 얼마나 떨어졌나(%) — 과열 판단.
- **롱 / 숏 / 현금**: 롱=사서 오르면 이익 / 숏=팔아서 내리면 이익 / 현금=아무것도 안 들고 쉼.

#### 🛠 모드·기능
- **옵티마이저 / 자동 탐색**: 여러 전략을 한 번에 돌려 보유를 이기는 순서로 줄세움.
- **워크포워드 검증** (Walk-Forward): 과거로 전략을 고르고 → '처음 보는' 미래 구간에 적용하길 반복. **과최적화(overfitting, 과거에만 잘 맞게 끼워맞춤)**를 걸러내는 검사.
  &nbsp;👉 비유: *시험 공부한 문제로 점수 내지 말고, 안 본 새 문제로 실력 확인하기.* 여기서 잘 나와야 '진짜' 전략.
- **과최적화** (Overfitting): 과거 데이터에만 기막히게 잘 맞도록 숫자를 맞춰버린 상태. 실제 미래에선 잘 안 통함. 위 워크포워드로 잡아냄.
- **합성 레버리지**: 실제 레버리지 ETF가 없던 옛날까지 "지수 × N배"를 시뮬레이션.

#### 🏷 종목 약어
- **SOXL**: 미국 반도체 지수의 **3배(↑) 레버리지 ETF** (변동 큼, 고위험).
- **SOXX**: 반도체 지수 ETF (1배).
- **TQQQ**: 나스닥100 **3배(↑)** ETF. / **QQQ**: 나스닥100 (1배).
- **^IXIC**: 나스닥 종합지수. / **NQ=F**: 나스닥100 선물(거의 24시간 거래).
- **레버리지 ETF**: 하루 수익의 배수(2·3배)를 추종. 오를 땐 크게 벌지만 **횡보·하락에선 가치가 급감**.
        """
    )

if not run:
    st.info("👈 왼쪽 사이드바에서 **▶ 실행** 버튼을 누르면 시작합니다. "
            "기본값(SOXX 신호 → SOXL · 일봉 전체 · 추세추종 250+버퍼5%)이 추천 설정이라 그대로 눌러도 됩니다.")
    st.stop()
...
if mode == "🔍 전략 자동 탐색":
    ...
    c1.metric("총 수익률", f"{best['총수익률'] * 100:,.0f}%",
              ...
              help="기간 전체 누적 손익. 아래 숫자는 '매수 후 보유' 대비 차이(%p).")
    c2.metric("CAGR", f"{best['CAGR'] * 100:,.1f}%", help="연평균 복리 수익률.")
    c3.metric("샤프", f"{best['샤프']:,.2f}", help="위험 대비 수익 효율. 높을수록 좋음(1↑ 양호).")
    c4.metric("MDD", f"{best['MDD'] * 100:,.0f}%", help="최대 낙폭(고점 대비 최대 하락). 0에 가까울수록 안전.")
    c5.metric("매매횟수", f"{best['매매횟수']}", help="기간 중 사고판 총 횟수. 적을수록 수수료·휩쏘 부담↓.")
